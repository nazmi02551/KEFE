...
void main() {
  test('Slice 18 contract preserves binary decision truthfulness', () {
    ...
    expect(
      contract['truthfulness']['canonical_option_values_unchanged'],
      isTrue,
    );
    expect(
      contract['truthfulness']['invented_percentage_for_binary_forbidden'],
      isTrue,
    );
    expect(
      contract['truthfulness']['pre_commit_collective_exposure_forbidden'],
      isTrue,
    );
    expect(contract['presentation']['continuous_animation_forbidden'], isTrue);
    expect(
      contract['foundation_reuse']['same_widget_in_production_and_preview'],
      isTrue,
    );
  });
  ...
  testWidgets('exactly two SINGLE_CHOICE options use the signature balance', (
    ...
    expect(
      find.byKey(const ValueKey('signature-balance-hero')),
      findsOneWidget,
    );
  ...
  testWidgets(
    'three-option choice does not opt into binary balance semantics',
    (tester) async {
      await _pumpQuestion(
        tester,
        question: const DecisionQuestion(
  ...
  testWidgets(
    'selected side is visual feedback without inventing a percentage',
    (tester) async {
      await _pumpQuestion(
        tester,
        question: const DecisionQuestion(
  ...
  testWidgets('Reduce Motion collapses hero selection transition to zero', (
    ...
    await _pumpBalance(tester, selectedIndex: 1, accessibleNavigation: true);
  ...
  testWidgets('hero compacts on a narrow phone and remains semantic', (
    ...
    expect(
      tester
          .getSize(find.byKey(const ValueKey('signature-balance-hero')))
          .height,
      122,
    );
    expect(find.bySemanticsLabel(RegExp(r'Sol seçenek')), findsOneWidget);
  });
