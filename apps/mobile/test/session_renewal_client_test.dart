...
void main() {
  test('proactive renewal is single-flight across concurrent requests', () async {
    final now = DateTime.utc(2026, 8, 27, 10);
    final store = MemoryCredentialStore();
    await store.writeBundle(bundle(expiresAt: now.add(const Duration(seconds: 30))));
  ...
  test('expired access renews once and retries the protected request once', () async {
    final now = DateTime.utc(2026, 8, 27, 10);
    final store = MemoryCredentialStore();
    await store.writeBundle(bundle(expiresAt: now.add(const Duration(days: 1))));
  ...
  test('active legacy access bootstraps without creating a new actor', () async {
    final store = MemoryCredentialStore();
    ...
    final client = MockClient((request) async {
      ...
      return http.Response(
        jsonEncode(bundleBody(rotationCounter: 0)),
        200,
      );
    });
    final coordinator = SessionRenewalCoordinator(
  ...
  test('guest continuity failure reaches repository as an explicit API state', () async {
    final now = DateTime.utc(2026, 8, 27, 10);
    final store = MemoryCredentialStore();
    await store.writeBundle(
