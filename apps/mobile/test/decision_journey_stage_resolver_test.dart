...
FlowRuntimeSnapshot runtime(List<FlowRuntimeStep> steps) => FlowRuntimeSnapshot(
  ...
  templateCode: 'TEST',
  templateVersionNo: 1,
  entryStepCode: steps.isEmpty ? '' : steps.first.code,
...
void main() {
  test('prefers the furthest runtime-ready step', () {
    ...
    final active = DecisionJourneyStageResolver.primary(snapshot);

    expect(active?.code, 'decision');
    expect(DecisionJourneyStageResolver.ordinal(snapshot, active!), 2);
  });
  ...
  test('falls back to final completed step for terminal journeys', () {
    final snapshot = runtime([
      step('context', FlowStepRuntimeState.completed),
  ...
  test('does not invent a stage for an empty runtime', () {
    expect(DecisionJourneyStageResolver.primary(runtime(const [])), isNull);
  });
}
