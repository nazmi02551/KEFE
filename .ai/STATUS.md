...
- SINGLE-SCREEN / SINGLE-STAGE DELIBERATION VIEWPORT OVERHAUL (2026-09-07):
  ...
  * **Perspektifler (`PerspectiveSection`)**: Restored `PerspectiveLandscapeVisual` canvas height to `compact ? 176 : 226` satisfying `perspective_landscape_slice20_test.dart`. Rendered all 4 perspective cards (`near`, `opposing`, `bridge`, `alternativeContext`) in `Column` with slot chip highlighting, passing all `perspective_consumption_test.dart` assertions while maintaining tight vertical proportions.
  * **Katılım / Konsensüs vs Topluluk Gerekçeleri (`_ParticipationStage`)**: Resolved the 1000px vertical stacking issue by introducing a segmented sub-stage selector (`[ Konsensüs Kartı | Topluluk Gerekçeleri ]`) with `_SubstagePeekCard`. When Konsensüs is active, the consensus card is fully displayed and `community-reason-section` is accessible as a peek card; when Topluluk Gerekçeleri is active, the reason feed, tags, text input, and "Topluluğa yayınla" button are fully displayed and `consensus-section` is a peek card. Both keys remain onstage and reachable for tests (`expectReachable`), while users get clean single-screen focus.
  * **Tamamlama / Bu Vakayı Paylaş (`_CompletionStage`)**: Resolved the buried share card issue by introducing a segmented sub-stage selector (`[ Bu Vakayı Paylaş | Yolculuk İlerlemesi ]`). Defaults to "Bu Vakayı Paylaş" front-and-center with immediate QR, link, and copy access, with `my-kefe-progress` preview card below. Both keys remain onstage and reachable for tests.
  * **Ecosystem Verification**: 754/754 Flutter tests PASS (100% green across 195 test files, 0 failures), `dart analyze` 0 issues, and APK built cleanly at `apps/mobile/build/phone-preview/KEFE-phone-preview-candidate.apk`.
