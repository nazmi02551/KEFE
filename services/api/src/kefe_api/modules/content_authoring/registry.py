...
class InMemoryContentAuthoringRegistry:
    """Configuration-backed validation used by authoring and bootstrap."""
    ...
    def _validate_sources_and_context(
        ...
        for source in version.sources:
            ...
            if (
                source.claim_status is not None
                and source.claim_status not in self.claim_states
            ):
                failures.append(
                    self._failure(
    ...
    def _validate_globalization(
        ...
        if invalid_countries:
            failures.append(
                self._failure(
                    ...
                    "Invalid ISO-3166 alpha-2 country codes: "
        ...
        active_questions = {
            question.stable_code: question for question in version.active_questions
        }
        ...
        for localization in version.localizations:
            ...
            referenced_questions = set(localization.question_prompts) | set(
                localization.option_labels
            )
            unknown_questions = sorted(
                referenced_questions - set(active_questions)
            )
            ...
            for stable_code, labels in localization.option_labels.items():
                ...
                canonical_options = {
                    str(value)
                    for value in question.response_schema.get("options", [])
                }
        ...
        if missing_reviews:
            failures.append(
                PublicationValidationFailure(
                    ...
                    detail="Missing required review modes: "
...
def default_authoring_registry() -> InMemoryContentAuthoringRegistry:
    return InMemoryContentAuthoringRegistry(
        base_formats=frozenset(
            {"TODAY", "DILEMMA", "LAB", "VS", "CALL", "DECIDE", "RETRO"}
        ),
        domains=frozenset(
            {
                "CIVIC_POLITICS",
        ...
        response_schema_validators={
            "SINGLE_CHOICE": lambda schema: (
                isinstance(schema.get("options"), list)
                and len(schema["options"]) >= 2
            ),
            "CONFIDENCE": lambda schema: (
                schema.get("min") == 1 and schema.get("max") == 10
            ),
        },
        allowed_modifiers={
            "TODAY": frozenset(
            ...
            "VS": frozenset(
                {"BLIND_FIRST", "CONFIDENCE_CAPTURE", "REASON_CAPTURE"}
            ),
            "CALL": frozenset(
                {
                    "BLIND_FIRST",
            ...
            "DECIDE": frozenset(
                {"EVOLVE", "CONFIDENCE_CAPTURE", "REASON_CAPTURE"}
            ),
            "RETRO": frozenset(
                {"OUTCOME_REVEAL", "CONFIDENCE_CAPTURE", "REASON_CAPTURE"}
            ),
        },
    )
