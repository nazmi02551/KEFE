        ...
        capabilities = tuple(
            CapabilityDefinition(
                ...
                compatible_primitive_codes=frozenset(
                    item.get("compatible_primitive_codes", [])
                ),
                config_schema_ref=item.get("config_schema_ref"),
                enabled=bool(item.get("enabled", True)),
            )
            for item in document.get("capabilities", [])
        )
        flow_templates = tuple(
            FlowTemplateDefinition(
                code=flow["code"],
                version_no=int(flow["version_no"]),
                label_key=flow["label_key"],
                entry_step_code=flow["entry_step_code"],
                steps=tuple(
                    FlowStepDefinition(
                        code=step["code"],
                        primitive_code=step["primitive_code"],
                        capability_codes=tuple(step.get("capability_codes", [])),
                        next_step_codes=tuple(step.get("next_step_codes", [])),
                        payload_schema_ref=step.get("payload_schema_ref"),
                    )
                    for step in flow.get("steps", [])
                ),
                enabled=bool(flow.get("enabled", True)),
            )
            for flow in document.get("flow_templates", [])
        )
