...
def _openapi_errors() -> list[str]:
    ...
    perspective_properties = schemas.get("PerspectiveItemResponse", {}).get("properties", {})
    ...
    if missing_perspective_fields:
        errors.append(
            "PerspectiveItemResponse missing fields: "
            + ", ".join(missing_perspective_fields)
        )
...
def _schema_errors() -> list[str]:
    ...
    required_fragments = {
        ...
        "CREATE TABLE content.perspective_item": "editorial Perspective persistence",
        "source_kind text NOT NULL CHECK (source_kind = 'EDITORIAL_HUMAN')": (
            "editorial-human Perspective provenance"
        ),
        "perspective_published_lookup_idx": "bounded published Perspective lookup",
    }
    return [
...
def main() -> None:
    ...
    print(
        ...
        "private reasons, Perspectives, identity, persistence and outbox invariants verified."
    )
