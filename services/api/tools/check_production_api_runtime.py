...
def main() -> None:
    ...
    _require(contract["capabilities"] == ["CAP-092", "CAP-123"], "capability binding")
    ...
    _require(config["required_persistence_backend"] == "postgres", "postgres production")
    ...
    _require(config["forbidden_database_host_suffixes"] == [".invalid"], "reserved host suffix")
    _require(config["development_merge_secret_forbidden"] is True, "development secret guard")
    _require(config["otp_capture_forbidden"] is True, "OTP capture guard")
    _require(config["otp_abuse_guard_off_forbidden"] is True, "OTP abuse guard")
    _require(config["committed_credentials_allowed"] is False, "credential policy")

    release = contract["schema_release"]
    _require(release["migration_command"] == "alembic upgrade head", "migration command")
    _require(release["mode"] == "EXPLICIT_PRE_DEPLOY", "migration mode")
    _require(release["auto_migrate_on_api_startup"] is False, "no auto migration")

    health = contract["health"]
    _require(health["liveness_path"] == "/health", "liveness path")
    _require(health["readiness_path"] == "/ready", "readiness path")
    _require(health["readiness_public_openapi"] is False, "readiness OpenAPI boundary")
    _require(health["postgres_readiness_query"] == "SELECT 1", "readiness query")
    _require(health["failure_status"] == 503, "readiness status")
    _require(health["dependency_error_details_exposed"] is False, "readiness secret boundary")
    ...
    _require("alembic upgrade head" not in dockerfile, "no auto migration in API startup")
    ...
    for forbidden in ("render.com", "railway", "supabase", "firebase", "neon.tech"):
        _require(forbidden not in dockerfile.lower(), f"provider lock in Dockerfile: {forbidden}")
    ...
    _require(
        production_api["status"] == contract["reachability_policy"]["current_inventory_status"],
        "reachability inventory must remain NOT_CONFIGURED before deployment",
    )
    _require(production_api["externally_reachable"] is False, "no external reachability claim")

    guards = contract["architecture_guards"]
    _require(guards["parallel_backend_allowed"] is False, "no parallel backend")
    _require(guards["hosting_vendor_in_domain_code_allowed"] is False, "no vendor in domain")
    _require(guards["preview_data_as_production_fallback_allowed"] is False, "preview isolation")
    _require(guards["commit_first_preserved"] is True, "Commit First")
    _require(guards["blind_first_preserved"] is True, "Blind First")
    _require(guards["immutable_published_case_version_preserved"] is True, "immutable CaseVersion")
    ...
    _require("does not make the API externally reachable" in adr, "reachability non-claim")
