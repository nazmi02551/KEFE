...
def main() -> int:
    ...
    errors.extend(
        _require(
            ...
            (
                ...
                "get_proposal(candidate_proposal_id)",
                "get_review_decision(candidate.id)",
                "ReviewedProposalBundle(",
                "dependency_ids=dependency_ids",
            ),
            label="Editorial Projection source bridge",
        )
    )
    errors.extend(
        _require(
            ...
            label="active persistence composition",
        )
    )
    errors.extend(
        _require(
    ...
    bridge_leaks = [fragment for fragment in bridge_forbidden if fragment in bridge]
    if bridge_leaks:
        errors.append(
            "forbidden provider/lifecycle dependency leaked into projection bridge: "
            + ", ".join(bridge_leaks)
        )
