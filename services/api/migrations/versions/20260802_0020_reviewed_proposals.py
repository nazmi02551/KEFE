...
def upgrade() -> None:
    ...
    op.execute(
        ...
        CREATE TABLE ingestion.ingestion_run (
            ...
            input_artifact_kind text NOT NULL CHECK (
                input_artifact_kind IN ('SOURCE_ARTIFACT','NORMALIZED_ARTIFACT')
            ),
        ...
        CREATE TABLE ingestion.stage_execution (
            ...
            executor_kind text NOT NULL CHECK (
                executor_kind IN (
                    ...
                    'HUMAN_ASSISTED','EXTERNAL_CAPABILITY'
                )
            ),
    ...
    op.execute(
        """
        CREATE INDEX ingestion_proposal_run_idx
